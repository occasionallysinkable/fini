...
| Push | Web Push with VAPID, a service worker, notification actions | The only channel, by decision. **Caveat that must be tested first:** on iOS, web push only works when the app has been added to the home screen. Verify this on the actual phone in WP7 before building anything on top of it. |
...
| 7 | Reminders you set | Presets and custom offsets. Service worker, VAPID, device records. The tick endpoint fires from the Cloudflare cron. Done and Later on the notification; Later expands into the three reasons; each reason is the snooze; the second snooze offers longer intervals. Completion on one device withdraws the rest. **Verify iOS home-screen push on the real phone before anything else in this package.** |
